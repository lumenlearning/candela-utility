<section id="post-<?php the_ID(); ?>" <?php post_class( array( 'top-block', 'clearfix', 'home-post' ) ); ?>>
		...
		<?php if ( ! is_single() ) { ?>
			<?php if ( ! is_user_logged_in() ) { ?>
				<a href="<?php echo wp_login_url(); ?>">
					<?php _e( 'login', 'pressbooks' ); ?>
				</a>
			<?php } else { ?>
				<a href="<?php echo wp_logout_url(); ?>">
					<?php _e( 'logout', 'pressbooks' ); ?>
				</a>
				<?php if ( is_super_admin() || is_user_member_of_blog() ) { ?>
					<a href="<?php echo get_option( 'home' ); ?>/wp-admin/admin.php?page=pressbooks">
						<?php _e( 'Admin', 'pressbooks' ); ?>
					</a>
				<?php } ?>
			<?php } ?>
		<?php } ?>
	...
	<div class="book-info">
		...
		<?php if ( ! empty( $metadata['pb_about_50'] ) ) { ?>
			<p>
				<?php echo pb_decode( $metadata['pb_about_50'] ); ?>
			</p>
		<?php } ?>
	</div> <!-- end .book-info -->
	...
	<div class="call-to-action-wrap">
		...
		<div class="call-to-action">
			<a class="btn red" href="<?php echo $first_chapter; ?>">
				<span class="read-icon"></span>
				<?php _e( 'Read', 'pressbooks' ); ?>
			</a>

			<?php if ( @array_filter( get_option( 'pressbooks_ecommerce_links' ) ) ) { ?>
				<!-- Buy -->
				<a class="btn black" href="<?php echo get_option( 'home' ); ?>/buy">
